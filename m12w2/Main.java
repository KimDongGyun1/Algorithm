...
public class Main {
	...
	static int[][] map;
	static boolean[][] visited; // 방문 체크를 위한 visited 배열 true, fasle
	public static void main(String[] args) throws IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
		
		StringTokenizer st = new StringTokenizer(br.readLine()); // 다음 
		N = Integer.parseInt(st.nextToken()); //4 - 상하
		M = Integer.parseInt(st.nextToken()); //6 - 좌우
	...
	private static void bfs(Dot dot) {
		...
		while(!q.isEmpty()) {
			...
			for (int k = 0; k < 4; k++) {
				...
				// 3. 전에 방문했던 곳인가. true면 방문x false면 방문o ??? 맞는
